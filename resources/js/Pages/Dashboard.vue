<template>
    <div class="bg-blue-500 py-4 px-4 flex items-center justify-between md:hidden">
        <!-- navbar starts here -->
        <div class="hamburger bg-standardPurple rounded-full py-1 px-1 cursor-pointer" ref="hamburger" @click="showMenu">
    ...
    <div class="flex min-h-screen">
        <!-- side bar -->
        <div
            class="w-0 md:w-1/5 bg-blue-300 md:block md:static h-screen sidebar fixed top-0 left-0 z-10 h-full overflow-x-hidden transition ease-in-out delay-150">
            <div class="closebutton absolute right-3 top-2 cursor-pointer md:hidden">
        ...
        <div class="w-full flex justify-center">
            <div class="w-3/5 mt-12">
                ...
                <form class="" @submit.prevent="form.post(route('update'))">
                    <label class="font-medium mt-6 mb-2 text-sm block">Full Name</label>

                    <input type="text" :value="data.original.data.user_details.name"
                    ...
                    <input type="email" :value="data.original.data.user_details.email"
                        class="border-2 border-standardPurple border-b-4 outline-0 rounded-md py-1 px-3 w-full" />
                    <br />
...
export default {
    ...
    setup() {
        ...
        return { form, destroy, data, hamburger, showMenu, logout }
    }
}
